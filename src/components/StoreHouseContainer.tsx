...
const StoreHouseContainer = ({name = 'none', borderColor = 'default', recentUpdate = new Date()} : StoreHouseProps) => {
    ...
    const formattedDate = monthNames[recentUpdate.getMonth()] + ' ' + 
            recentUpdate.getDate() + ' ' + 
            recentUpdate.getFullYear() + ' at ' +
            String(recentUpdate.getHours()).padStart(2, '0') + ':' + 
            String(recentUpdate.getMinutes()).padStart(2, '0');

    return( 
        <div className={`relative w-full flex justify-between h-64 bg-slate-600 border-4 ${colors[borderColor]} rounded-2xl p-4 hover:bg-slate-500
                         md:h-96`}>                    
            <div>
            ...
            <span className=" absolute left-2 bottom-2 text-style-2">Most recent update: {`${formattedDate}`}</span>
...
StoreHouseContainer.propTypes = {
    name: PropTypes.string,
    borderColor: PropTypes.oneOf(['red' , 'green' , 'default']),
    members: PropTypes.array,
    recentUpdate: PropTypes.instanceOf(Date)
}

export default StoreHouseContainer